...
func Start() {
	...
	loadedConfiguration, err := configuration.Configure()
	if err != nil {
		log.Fatalln(err)
	}
	...
	if err := server.Shutdown(ctx); err != nil {
		log.Fatalf("something went wrong in shutting down the server, %s", err)
	}
...
func homeHandler(w http.ResponseWriter, r *http.Request) {
	http.Redirect(w, r, "/repositories", http.StatusPermanentRedirect)
}
